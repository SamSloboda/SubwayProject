...
public class DataCleaningMapper1 extends Mapper<LongWritable, Text, NullWritable, Text> {
    ...
    public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        ...
        if(!line[3].equals("")){     //checking for missing date ==> DROPPING IF MISSING
            date = line[3];
        }else{
            return;
        }

        if(!line[4].equals("")){    //checking for missing time ==> DROPPING IF MISSING
            time = line[4];
        }else{
            return;
        }

        if(!line[107].equals("") && !line[108].equals("")){ //checking if the record has geolocation. If not, dropping imidiatelly.
            xcoord = line[107];
            ycoord = line[108];
        }else{
            return;
    ...
    */
    ...
    crimeIDcrimeDescriptions.put("113","FORCIBLE TOUCHING");
        ...
        if(line[44].equalsIgnoreCase("Y") || line[44].equals("")){
            violentCrime = true;
        }
